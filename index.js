import{a as d,P as x,i as S}from"./assets/vendor-Cir5bjeI.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))a(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function r(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(s){if(s.ep)return;s.ep=!0;const i=r(s);fetch(s.href,i)}})();const l={exampele:document.querySelector(".exampele"),artistsList:document.querySelector(".artists__list"),artistsLoadButton:document.querySelector(".artists__load-btn"),artistModalElems:{artistModalEl:document.querySelector(".js-artist-modal"),closeArtistModalBtnEl:document.querySelector("[data-artist-modal-close]"),artistModalInnerEl:document.querySelector(".js-artist-modal-inner"),artistModalBodyEl:document.querySelector(".js-artist-modal-body"),artistModalLoaderEl:document.querySelector(".js-artist-modal-loader")}};d.defaults.baseURL="https://sound-wave.b.goit.study/api/";async function j(t=1){try{const{data:e}=await d.get("artists",{params:{limit:8,page:t}});return e}catch(e){throw console.error("Error fetching artists:",e.message),e}}//! ============================================================================
const I=async t=>(await d.get(`artists/${t}/albums`)).data;function y(t){document.getElementById(t).classList.remove("hidden")}function p(t){document.getElementById(t).classList.add("hidden")}async function f(t){try{const e=await j(t);y("artists__loader");const r=e.artists.map(a=>`
        ...
        `).join("");return l.artistsList.innerHTML=r,p("artists__loader"),e}catch(e){p("artists__loader"),console.error("Error in createArtistsList:",e)}}const q=document.getElementById("pagination");let g=null;async function C(){const t=await f(1);g=new x(q,{totalItems:t.totalArtists,itemsPerPage:t.limit,visiblePages:5,centerAlign:!0,template:{page:'<a href="#" class="custom-page-btn">{{page}}</a>',currentPage:'<strong class="custom-current">{{page}}</strong>',moveButton:'<a href="#" class="custom-move-btn tui-{{type}}"><span class="icon-{{type}}">{{type}}</span></a>',disabledMoveButton:'<span class="custom-move-btn disabled custom-{{type}}"><span class="icon-{{type}}">{{type}}</span></span>',moreButton:'<a href="#" class="custom-ellip">...</a>'}}),g.on("afterMove",async e=>{const r=e.page;y("artists__loader"),await f(r)})}C();const h="/seven-friends-of-a-FullStack/assets/sprite-mTqa9sFe.svg";let b;//! ============================================================================
...
const D=t=>{const{strArtist:e,strArtistThumb:r,intFormedYear:a,intDiedYear:s="",strGender:i,intMembers:o,strCountry:E,strBiographyEN:A,genres:T}=t,w=H({intFormedYear:a,intDiedYear:s}),m=o&&o>1,$=U([{title:"Years active",key:"activeYears",value:w},{title:"Sex",key:"strGender",value:i,isBand:m},{title:"Members",key:"intMembers",value:o,isBand:m},{title:"Country",key:"strCountry",value:E},{title:"Biography",key:"strBiographyEN",value:A}]),B=K(T);return`<section class="about-artist js-about-artist">
                ...
                <div class="about-artist__content">
                    ...
                    <ul class="about-artist__info-list">
                        ...
                        </li>`;return r?o:""}function U(t){return t.map(F).join(`
`)}//! ============================================================================
const R=t=>`<li class="about-artist__music-genre">${t}</li>`;function K(t){return(t==null?void 0:t.length)>0?t.map(R).join(`
`):""}//! ============================================================================
const V=t=>{const e=D(t);c.artistModalInnerEl.insertAdjacentHTML("afterbegin",e)};//! ============================================================================
const W=t=>{const e=J(t);c.artistModalInnerEl.insertAdjacentHTML("beforeend",e)};function J(t){return`<section class="artist-albums js-artist-modal-albums">
...
function it(){_=window.scrollY,document.body.style.top=`-${_}px`,document.body.classList.add("scroll-lock"),n.artistModalEl.classList.add("artist-modal--is-open"),document.addEventListener("keydown",k),window.addEventListener("resize",L),n.artistModalEl.addEventListener("click",M),n.closeArtistModalBtnEl.addEventListener("click",u),n.artistModalBodyEl.scrollTop=0}function u(){n.artistModalEl.classList.remove("artist-modal--is-open"),document.body.classList.remove("scroll-lock");const t=Math.abs(parseInt(document.body.style.top||"0",10));document.body.style.top="",window.scrollTo(0,t),n.artistModalInnerEl.innerHTML="",document.removeEventListener("keydown",k),window.removeEventListener("resize",L),n.artistModalEl.removeEventListener("click",M),n.closeArtistModalBtnEl.removeEventListener("click",u)}//! ============================================================================
function k(t){t.key==="Escape"&&u()}//! ============================================================================
function M(t){t.target===n.artistModalEl&&u()}const{artistsList:ot}=l;//! ============================================================================
